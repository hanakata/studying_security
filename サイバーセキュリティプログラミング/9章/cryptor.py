...
from Cryptodome.Cipher import AES, PKCS1_0AEP
...
def get_rsa_cipher(keytype):
    ...
    return (PKCS1_0AEP.new(rsakey), rsakey.size_in_bytes())
...
def decrypt(encrypted):
    ...
    cipeher_rsa, keysize_in_bytes = get_rsa_cipher('pri')
    ...
    session_key = cipeher_rsa.decrypt(encrypted_session_key)
    cipeher_aes = AES.new(session_key, AES.MODE_EAX, nonce)
    decrypted = cipeher_aes.decrypt_and_verify(ciphertext, tag)
